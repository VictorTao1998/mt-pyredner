...
def main():
    ...
    depth_name = args.left_name
    depth_name = depth_name.replace("_half", f"_n{noise_scale:.1f}_uq{args.unique}_ss")
    depth_name = depth_name.replace("_irL_", "_depth_")

    timestamp = time.strftime("%y-%m-%d_%H-%M-%S")
    run_name = "{:s}".format(timestamp)
    log_file = open(data_dir / f"ss_depth_{depth_name[:-4]}_{run_name}.txt", "w")
    ...
    for img_dir in tqdm(img_dirs):
        index += 1
        if args.skip:
            if os.path.exists(os.path.join(img_dir, depth_name)):
                depth = cv2.imread(os.path.join(img_dir, depth_name), cv2.IMREAD_UNCHANGED)
                try:
                    depth = cv2.resize(depth, (160, 90), interpolation=cv2.INTER_LINEAR)
                    log_file.write(f"{img_dir} depth exists\n")
                    print(f"{img_dir} depth exists")
                    continue
                except:
                    log_file.write(f"{img_dir} depth corrupted\n")
                    print(f"{img_dir} depth corrupted")
