...
def render_scene(
    ...
    if rand_env:
        ...
        plight1 = scene.add_point_light([1.0, 0.2, 2.5], [80, 80, 80])
        plight2 = scene.add_point_light([1.0, -2.7, 2.5], [80, 80, 80])
        plight3 = scene.add_point_light([1.0, -5.6, 2.5], [80, 80, 80])
        plight4 = scene.add_point_light([1.0, 3.1, 2.5], [80, 80, 80])
        plight5 = scene.add_point_light([1.0, 6.0, 2.5], [80, 80, 80])

        # change light
        def lights_on():
            plight1.set_color([80, 80, 80])
            plight2.set_color([80, 80, 80])
            plight3.set_color([80, 80, 80])
            plight4.set_color([80, 80, 80])
            plight5.set_color([80, 80, 80])
            alight.set_color([0.0, 0.0, 0.0])

        def lights_off():
            p_scale = 0.05
            scene.set_ambient_light([0.01, 0., 0.])
            plight1.set_color(plight1.color * p_scale)
            plight2.set_color(plight2.color * p_scale)
            plight3.set_color(plight3.color * p_scale)
            plight4.set_color(plight4.color * p_scale)
            plight5.set_color(plight5.color * p_scale)
            alight.set_color([150, 0, 0])
    ...
    if primitives:
        ...
        num_asset = random.randint(THIN_PRIMITIVE_MIN, THIN_PRIMITIVE_MAX)
        primitive_info = {}
        for i in range(num_asset):
            info = load_random_thin_primitives(scene, renderer=renderer, idx=i, trans_prob=trans_prob)
            primitive_info.update(info)
    else:
    ...
    for view_id in range(num_views):
        ...
        if not os.path.exists(os.path.join(folder_path, f"{spp:04d}_irR_{material_name}_den{denoise}_half.png")):
            # begin rendering
            lights_on()
            scene.update_render()
            ...
            plt.imsave(os.path.join(folder_path, f"{spp:04d}_rgb_{material_name}_den{denoise}.png"), p)

            # Render multi-view RGB camera
            cam_irl.take_picture()
            p = cam_irl.get_color_rgba()
            plt.imsave(os.path.join(folder_path, f"{spp:04d}_rgbL_{material_name}_den{denoise}.png"), p)

            cam_irr.take_picture()
            p = cam_irr.get_color_rgba()
            plt.imsave(os.path.join(folder_path, f"{spp:04d}_rgbR_{material_name}_den{denoise}.png"), p)
            plt.close("all")

            if not alight_pos_list:
                ...
                irl = p_l[::2, ::2]
                irr = p_r[::2, ::2]
                cv2.imwrite(os.path.join(folder_path, f"{spp:04d}_irL_{material_name}_den{denoise}_half.png"), irl)
                cv2.imwrite(os.path.join(folder_path, f"{spp:04d}_irR_{material_name}_den{denoise}_half.png"), irr)
            else:
                for alight_index, alight_pos in enumerate(alight_pos_list):
                    apos = cam_mount.get_pose().to_transformation_matrix()[:3, :3] @ mount_T
                    apos = t3d.quaternions.mat2quat(apos)
                    alight.set_pose(Pose(cam_mount.get_pose().p + alight_pos, apos))
                    lights_off()
                    scene.update_render()
                    ...
                    cv2.imwrite(os.path.join(folder_path,
                                             f"{spp:04d}_ir{alight_index:02d}L_{material_name}_den{denoise}_half.png"),
                                irl)
                    cv2.imwrite(os.path.join(folder_path,
                                             f"{spp:04d}_ir{alight_index:02d}R_{material_name}_den{denoise}_half.png"),
                                irr)
        ...
        if not os.path.exists(os.path.join(folder_path, "meta.pkl")):
            # Save scene info
            scene_info = {
            ...
            if primitives or primitives_v2 or thin:
...
def render_gt_depth_label(
    ...
    for view_id in range(num_views):
        ...
        p_l = cv2.GaussianBlur(p_l, (3, 3), 1)
        irl = p_l[::2, ::2]
        cv2.imwrite(os.path.join(folder_path, f"0000_irL_half.png"), irl)
        ...
        p_r = cv2.GaussianBlur(p_r, (3, 3), 1)
        irr = p_r[::2, ::2]
        cv2.imwrite(os.path.join(folder_path, f"0000_irR_half.png"), irr)
        ...
        if not (primitives or primitives_v2 or thin):
            # render label image
