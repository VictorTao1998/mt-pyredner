# by Jet <sgh2aw5eg89ew8@jetd.me>, Jan 2021
...
# def sim_ir_noise(
#         img: np.ndarray,
#         scale: float = 0.8, blur_ksize: int = 3, blur_ksigma: float = 0.03,
#         speckle_shape: float = 4.54, speckle_scale: float = 0.196,
#         gaussian_mu: float = -0.126, gaussian_sigma: float = 10.4,
#         seed: int = 1234567
# ) -> np.ndarray:
def sim_ir_noise(
    img: np.ndarray,
...
def get_census(img, wsize=7):
    ...
    offsets = [(u, v) for v in range(wsize) for u in range(wsize) if not u == v == whalf]

    for u, v in offsets:
        census = (census << 1) | (img[v : v + h - 2 * whalf, u : u + w - 2 * whalf] >= center)
    ...
    if lr_consistency:
        if ximgproc is not None:
            ...
            displ = wls_filter.filter(
                disparity_map_left=displ, disparity_map_right=dispr, left_view=imgl, right_view=imgr
            )
...
def calc_main_depth_from_left_right_ir(
    ...
    **kwargs,
    ...
    _, _, q = calc_rectified_stereo_pair(ir_l, ir_r, k_l.astype(np.float), k_r.astype(np.float), rt_lr.astype(np.float))

    disp = calc_disparity(
        ir_l, ir_r, method, lr_consistency=lr_consistency, ndisp=ndisp, use_census=use_census, census_wsize=census_wsize
    )
    ...
    if register_depth:
        depth = cv2.rgbd.registerDepth(k_l, k_main, None, rt_mainl, depth, (w, h), depthDilation=True)
        depth[np.isnan(depth)] = 0
        depth[np.isinf(depth)] = 0
        depth[depth < 0] = 0
