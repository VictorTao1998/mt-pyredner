...
def render_scene(
    ...
    for view_id in range(num_views):
        ...
        if fixed_angle:
            ...
            cam_mount.set_pose(cv2ex2pose(cam_extrinsic_list[view_id]))
<<<<<<< HEAD
            apos = cam_irl.get_pose().to_transformation_matrix()[:3, :3] @ mount_T
            apos = t3d.quaternions.mat2quat(apos)
            alight.set_pose(Pose(cam_irl.get_pose().p, apos))

=======
>>>>>>> 0936088e8f9b2cd92c79cd2c500032795a0938af
        else:
        ...
        if not os.path.exists(os.path.join(folder_path, f"{spp:04d}_irR_kuafu_half_no_ir.png")):
            ...
            if not alight_pos_list:
                ...
                for alight_index, alight_pos in enumerate(alight_pos_list):
                    ...
                    irl = p_l[::2, ::2]
                    irr = p_r[::2, ::2]
                    cv2.imwrite(os.path.join(folder_path, f"{spp:04d}_ir{alight_index:02d}L_kuafu_half.png"), irl)
                    cv2.imwrite(os.path.join(folder_path, f"{spp:04d}_ir{alight_index:02d}R_kuafu_half.png"), irr)
