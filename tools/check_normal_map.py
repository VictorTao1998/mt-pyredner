...
def sub_main_rgb(prefix_list):
    n = len(prefix_list)
    start = time.time()
    for idx in range(n):
        p = prefix_list[idx]
        if os.path.exists(os.path.join(args.data_folder, p, "normal.png")):
            try:
                normal_map = cv2.imread(os.path.join(args.data_folder, p, "normal.png"), cv2.IMREAD_UNCHANGED)
                normal_map = (normal_map.astype(float)) / 1000.0
                logger.info(f"{p} normal map is good {idx}/{n} time: {time.time() - start:.2f}s")
            except:
                logger.info(f"{os.path.join(args.data_folder, p, 'normal.png')} is corrupted.")
                depth = cv2.imread(os.path.join(args.data_folder, p, "depth.png"), cv2.IMREAD_UNCHANGED)
                depth = (depth.astype(float)) / 1000
                depth = cv2.resize(depth, (960, 540), interpolation=cv2.INTER_NEAREST)
                meta = load_pickle(os.path.join(args.data_folder, p, "meta.pkl"))
                intrinsic = meta["intrinsic"]
                intrinsic[:2] /= 2
                normal_map = cal_normal_map(depth, intrinsic, max_nn=50)
                normal_map_colored = ((normal_map + 1) * 127.5).astype(np.uint8)
                cv2.imwrite(os.path.join(args.data_folder, p, "normal_colored.png"), normal_map_colored)
                normal_map = ((normal_map + 1) * 1000).astype(np.uint16)
                cv2.imwrite(os.path.join(args.data_folder, p, "normal.png"), normal_map)
                logger.info(f"Generating {p} normal map {idx}/{n} time: {time.time() - start:.2f}s")
        else:
            logger.warning(f"No normal map in {os.path.join(args.data_folder, p)}")
            # depth = cv2.imread(os.path.join(args.data_folder, p, "depth.png"), cv2.IMREAD_UNCHANGED)
