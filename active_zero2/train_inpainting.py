...
from active_zero2.utils.solver import (
    build_d_optimizer,
    build_inpaint_optimizer,
    build_lr_scheduler,
    build_stereo_optimizer,
)
...
if __name__ == "__main__":
    ...
    assert cfg.MODEL_TYPE in ("PSMNetInpainting",)
    ...
    inpaint_optimizer = build_inpaint_optimizer(cfg, model)
    stereo_optimizer = build_stereo_optimizer(cfg, model)
    # Build lr_scheduler
    scheduler = build_lr_scheduler(cfg, stereo_optimizer)
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # Forward
        sim_pred_dict = model_parallel(sim_data_batch, real=False)
        ...
        # Forward
        real_pred_dict = model_parallel(real_data_batch, real=(cur_iter > cfg.ADV_ITER))
        ...
        if cur_iter > cfg.ADV_ITER:
            # train D
            dis_real_loss, dis_fake_loss = model.compute_D_loss(real_data_batch, real_pred_dict)
            loss_dict["loss_D_real"] = dis_real_loss
            loss_dict["loss_D_fake"] = dis_fake_loss
            d_loss = (dis_real_loss + dis_fake_loss) / 2
            d_loss.backward()
            d_optimizer.step()

            if cfg.LOSS.ADV > 0:
                adv_loss = model.compute_G_loss(real_data_batch, real_pred_dict)
        ...
        if log_period > 0 and (cur_iter % log_period == 0 or cur_iter == 1):
            logger.info(
                train_meters.delimiter.join(
                    ...
                    lr=stereo_optimizer.param_groups[0]["lr"],
            ...
            summary_writer.add_scalar(
                "train/learning_rate", stereo_optimizer.param_groups[0]["lr"], global_step=cur_iter
            )
