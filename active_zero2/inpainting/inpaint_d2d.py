...
def main():
    ...
    for data in tqdm(dataset):
        ...
        if args.original:
            ...
            depth = np.load(curr_pred_dir / "depth_pred.npy")
            pred_dict = {"depth": depth}
            data = {k: v.unsqueeze(0) for k, v in data.items() if isinstance(v, torch.Tensor)}
            data["dir"] = sc
            curr_metric = metric.compute(data, pred_dict, save_folder="", real_data=True)
            logger.info("\t".join([f"{k}: {v:.3f}" for k, v in curr_metric.items()]))
            continue
        view_folder = output_folder / sc
        view_folder.makedirs_p()
        curr_pred_dir = pred_folder / f"{sc}_real"
        if (view_folder / "depth_d2d_fusion.npy").exists():
            # continue
            depth_d2d_fusion = np.load(view_folder / "depth_d2d_fusion.npy")
        else:
            depth_pred = np.load(curr_pred_dir / "depth_pred.npy")
            ...
            if args.gt_mask:
                ...
                normal_fitting = cv2.imread(curr_pred_dir / "normal_pred_u16.png", cv2.IMREAD_UNCHANGED)
                ...
                if args.gt_normal:
                    ...
                    normal_pred = (normal_pred.astype(float)) / 1000.0 - 1
                    #
                    # conf_mask_bak = np.logical_or(conf_mask_bak, np.abs(normal_pred[..., 2]) < 0.2)
                    # conf_mask_bak = np.logical_or(conf_mask_bak, np.abs(normal_fitting[..., 2]) < 0.2)
                    # plt.imsave(
                    #     view_folder / "conf_mask3.png", conf_mask_bak.astype(float), vmin=0.0, vmax=1.0, cmap="jet"
                    # )
            ...
            cv2.imwrite(view_folder / "depth_pred_u16.png", (depth_pred * 4000).astype(np.uint16))
            ...
            o = (
                f"/home/rayu/Projects/cleargrasp/api/depth2depth/gaps/bin/x86_64/depth2depth"
                f" {view_folder}/depth_pred_u16.png {view_folder}/depth_pred_u16_out.png -xres 960 -yres 540"
            ...
            depth_d2d = cv2.imread(view_folder / "depth_pred_u16_out.png", cv2.IMREAD_UNCHANGED)
