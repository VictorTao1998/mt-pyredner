								  ...
								  const cv::Mat &plane_left, const cv::Mat &plane_right, const cv::Mat &edge_left, const cv::Mat &edge_right)
{
	...
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			disp_left_[i * width + j] = (float)disp_left.at<uint16>(i, j) / 500.0;
			disp_right_[i * width + j] = (float)disp_right.at<uint16>(i, j) / 500.0;
			disp_conf_left_[i * width + j] = (disp_left_[i * width + j] > 0.01);
			disp_conf_right_[i * width + j] = (disp_right_[i * width + j] > 0.01);
		}
	}
	...
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			if (plane_left.at<Vec3w>(i, j)[2] == 0)
			{
				plane_conf_left_[i * width + j] = false;
			}
			else
			{
				plane_left_[i * width + j] = DisparityPlane(((float32)plane_left.at<Vec3w>(i, j)[0] - 10000.0) / 20000.0,
															((float32)plane_left.at<Vec3w>(i, j)[1] - 10000.0) / 20000.0,
															((float32)plane_left.at<Vec3w>(i, j)[2] - 10000.0) / 20000.0);
				plane_conf_left_[i * width + j] = true;
			}
			if (plane_right.at<Vec3w>(i, j)[2] == 0)
			{
				plane_conf_right_[i * width + j] = false;
			}
			else
			{
				plane_right_[i * width + j] = DisparityPlane(((float32)plane_right.at<Vec3w>(i, j)[0] - 10000.0) / 20000.0,
															 ((float32)plane_right.at<Vec3w>(i, j)[1] - 10000.0) / 20000.0,
															 ((float32)plane_right.at<Vec3w>(i, j)[2] - 10000.0) / 20000.0);
...
bool PatchMatchStereo::Propogate(float32 *disp_prop_left)
{
	if (!is_initialized_)
	{
		return false;
