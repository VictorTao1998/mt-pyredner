...
class feature_extraction(nn.Module):
    def __init__(self, concat_feature=False, concat_feature_channel=12):
        ...
        self.upconv6 = nn.Sequential(nn.Upsample(scale_factor=2), convbn(512, 256, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.iconv5 = nn.Sequential(convbn(512, 256, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.upconv5 = nn.Sequential(nn.Upsample(scale_factor=2), convbn(256, 192, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.iconv4 = nn.Sequential(convbn(384, 192, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.upconv4 = nn.Sequential(nn.Upsample(scale_factor=2), convbn(192, 128, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.iconv3 = nn.Sequential(convbn(256, 128, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.upconv3 = nn.Sequential(nn.Upsample(scale_factor=2), convbn(128, 64, 3, 1, 1, 1), nn.ReLU(inplace=True))
        self.iconv2 = nn.Sequential(convbn(128, 64, 3, 1, 1, 1), nn.ReLU(inplace=True))
        ...
        self.gw2 = nn.Sequential(
            convbn(64, 80, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(80, 80, kernel_size=1, padding=0, stride=1, bias=False),
        )

        self.gw3 = nn.Sequential(
            convbn(128, 160, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(160, 160, kernel_size=1, padding=0, stride=1, bias=False),
        )

        self.gw4 = nn.Sequential(
            convbn(192, 160, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(160, 160, kernel_size=1, padding=0, stride=1, bias=False),
        )

        self.gw5 = nn.Sequential(
            convbn(256, 320, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(320, 320, kernel_size=1, padding=0, stride=1, bias=False),
        )

        self.gw6 = nn.Sequential(
            convbn(512, 320, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(320, 320, kernel_size=1, padding=0, stride=1, bias=False),
        )
        ...
        if self.concat_feature:
            ...
            self.concat4 = nn.Sequential(
                convbn(192, 128, 3, 1, 1, 1),
                nn.ReLU(inplace=True),
                nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1, bias=False),
            )

            self.concat5 = nn.Sequential(
                convbn(256, 128, 3, 1, 1, 1),
                nn.ReLU(inplace=True),
                nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1, bias=False),
            )

            self.concat6 = nn.Sequential(
                convbn(512, 128, 3, 1, 1, 1),
                nn.ReLU(inplace=True),
                nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1, bias=False),
            )
    ...
    def forward(self, x):
        ...
        if not self.concat_feature:
            ...
            return {
                ...
                "concat_feature5": concat_feature5,
                "concat_feature6": concat_feature6,
            }
...
class hourglassup(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Conv3d(in_channels, in_channels * 2, kernel_size=3, stride=2, padding=1, bias=False)

        self.conv2 = nn.Sequential(convbn_3d(in_channels * 2, in_channels * 2, 3, 1, 1), nn.ReLU(inplace=True))

        self.conv3 = nn.Conv3d(in_channels * 2, in_channels * 4, kernel_size=3, stride=2, padding=1, bias=False)
...
class cfnet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        self.dres0_5 = nn.Sequential(
            convbn_3d(self.num_groups + self.concat_channels * 2, 64, 3, 1, 1),
            nn.ReLU(inplace=True),
            convbn_3d(64, 64, 3, 1, 1),
            nn.ReLU(inplace=True),
        )

        self.dres1_5 = nn.Sequential(convbn_3d(64, 64, 3, 1, 1), nn.ReLU(inplace=True), convbn_3d(64, 64, 3, 1, 1))
        ...
        self.dres1_6 = nn.Sequential(convbn_3d(64, 64, 3, 1, 1), nn.ReLU(inplace=True), convbn_3d(64, 64, 3, 1, 1))
        ...
        self.confidence0_s3 = nn.Sequential(
            convbn_3d(self.num_groups + self.concat_channels * 2 + 1, 32, 3, 1, 1),
            nn.ReLU(inplace=True),
            convbn_3d(32, 32, 3, 1, 1),
            nn.ReLU(inplace=True),
        )

        self.confidence1_s3 = nn.Sequential(
            convbn_3d(32, 32, 3, 1, 1), nn.ReLU(inplace=True), convbn_3d(32, 32, 3, 1, 1)
        )
        ...
        self.confidence0_s2 = nn.Sequential(
            convbn_3d(self.num_groups // 2 + self.concat_channels + 1, 16, 3, 1, 1),
            nn.ReLU(inplace=True),
            convbn_3d(16, 16, 3, 1, 1),
            nn.ReLU(inplace=True),
        )

        self.confidence1_s2 = nn.Sequential(
            convbn_3d(16, 16, 3, 1, 1), nn.ReLU(inplace=True), convbn_3d(16, 16, 3, 1, 1)
        )
        ...
        self.confidence_classif0_s3 = nn.Sequential(
            convbn_3d(32, 32, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(32, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )

        self.confidence_classif1_s3 = nn.Sequential(
            convbn_3d(32, 32, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(32, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )

        self.confidence_classifmid_s3 = nn.Sequential(
            convbn_3d(32, 32, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(32, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )

        self.confidence_classif0_s2 = nn.Sequential(
            convbn_3d(16, 16, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(16, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )

        self.confidence_classif1_s2 = nn.Sequential(
            convbn_3d(16, 16, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(16, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )

        self.confidence_classifmid_s2 = nn.Sequential(
            convbn_3d(16, 16, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv3d(16, 1, kernel_size=3, padding=1, stride=1, bias=False),
        )
    ...
    def generate_disparity_samples(self, min_disparity, max_disparity, sample_count=12):
        ...
        disparity_samples = torch.cat(
            (torch.floor(min_disparity), disparity_samples, torch.ceil(max_disparity)), dim=1
        ).long()  # disparity level = sample_count + 2
    ...
    def forward(self, data_batch):
        ...
        if self.use_concat_volume:
            concat_volume4 = build_concat_volume(
                features_left["concat_feature4"], features_right["concat_feature4"], self.maxdisp // 8
            )
            concat_volume5 = build_concat_volume(
                features_left["concat_feature5"], features_right["concat_feature5"], self.maxdisp // 16
            )
            concat_volume6 = build_concat_volume(
                features_left["concat_feature6"], features_right["concat_feature6"], self.maxdisp // 32
            )
        ...
        maxdisparity_s3 = F.upsample(
            maxdisparity_s3 * 2, [left.size()[2] // 4, left.size()[3] // 4], mode="bilinear", align_corners=True
        )
        mindisparity_s3 = F.upsample(
            mindisparity_s3 * 2, [left.size()[2] // 4, left.size()[3] // 4], mode="bilinear", align_corners=True
        )

        mindisparity_s3_1, maxdisparity_s3_1 = self.generate_search_range(
            self.sample_count_s3 + 1, mindisparity_s3, maxdisparity_s3, scale=2
        )
        disparity_samples_s3 = self.generate_disparity_samples(
            mindisparity_s3_1, maxdisparity_s3_1, self.sample_count_s3
        ).float()
        confidence_v_concat_s3, _ = self.cost_volume_generator(
            features_left["concat_feature3"], features_right["concat_feature3"], disparity_samples_s3, "concat"
        )
        confidence_v_gwc_s3, disparity_samples_s3 = self.cost_volume_generator(
            features_left["gw3"], features_right["gw3"], disparity_samples_s3, "gwc", self.num_groups
        )
        ...
        maxdisparity_s2 = F.upsample(
            maxdisparity_s2 * 2, [left.size()[2] // 2, left.size()[3] // 2], mode="bilinear", align_corners=True
        )
        mindisparity_s2 = F.upsample(
            mindisparity_s2 * 2, [left.size()[2] // 2, left.size()[3] // 2], mode="bilinear", align_corners=True
        )

        mindisparity_s2_1, maxdisparity_s2_1 = self.generate_search_range(
            self.sample_count_s2 + 1, mindisparity_s2, maxdisparity_s2, scale=1
        )
        disparity_samples_s2 = self.generate_disparity_samples(
            mindisparity_s2_1, maxdisparity_s2_1, self.sample_count_s2
        ).float()
        confidence_v_concat_s2, _ = self.cost_volume_generator(
            features_left["concat_feature2"], features_right["concat_feature2"], disparity_samples_s2, "concat"
        )
        confidence_v_gwc_s2, disparity_samples_s2 = self.cost_volume_generator(
            features_left["gw2"], features_right["gw2"], disparity_samples_s2, "gwc", self.num_groups // 2
        )
        ...
        if self.training:
            ...
            cost0_4 = F.upsample(
                cost0_4, [self.maxdisp, left.size()[2], left.size()[3]], mode="trilinear", align_corners=True
            )
            cost0_4 = torch.squeeze(cost0_4, 1)
            pred0_4 = F.softmax(cost0_4, dim=1)
            pred0_4 = disparity_regression(pred0_4, self.maxdisp).unsqueeze(1)

            cost1_4 = F.upsample(
                cost1_4, [self.maxdisp, left.size()[2], left.size()[3]], mode="trilinear", align_corners=True
            )
            cost1_4 = torch.squeeze(cost1_4, 1)
            pred1_4 = F.softmax(cost1_4, dim=1)
            pred1_4 = disparity_regression(pred1_4, self.maxdisp).unsqueeze(1)
            ...
            pred0_s3 = F.upsample(pred0_s3 * 4, [left.size()[2], left.size()[3]], mode="bilinear", align_corners=True)
            ...
            predmid_s3 = F.upsample(
                predmid_s3 * 4, [left.size()[2], left.size()[3]], mode="bilinear", align_corners=True
            )
            # predmid_s3 = torch.squeeze(predmid_s3, 1)

            pred1_s3_up = F.upsample(
                pred1_s3 * 4, [left.size()[2], left.size()[3]], mode="bilinear", align_corners=True
            )
            ...
            predmid_s2 = F.upsample(
                predmid_s2 * 2, [left.size()[2], left.size()[3]], mode="bilinear", align_corners=True
            )
            ...
            return {"disp_preds": [pred1_s2, pred1_s3_up, pred2_s4]}
    ...
    def compute_disp_loss(self, data_batch, pred_dict):
        ...
        loss_disp = 0.0
        for disp_pred, loss_weight in zip(pred_dict["disp_preds"], weights):
            loss_disp += loss_weight * F.smooth_l1_loss(disp_pred[mask], disp_gt[mask], reduction="mean")
    ...
    def compute_reproj_loss(self, data_batch, pred_dict, use_mask: bool, patch_size: int, only_last_pred: bool):
        ...
        if only_last_pred:
            loss_reproj = compute_reproj_loss_patch(
                ...
                pred_disp_l=pred_dict["disp_preds"][-1],
                mask=mask,
                ps=patch_size,
            )

            # for distribution training
            for p in pred_dict["disp_preds"][:-1]:
                loss_reproj += p.mean() * 0.0
