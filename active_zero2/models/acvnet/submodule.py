...
class Conv2x(nn.Module):
    def __init__(
        ...
        if deconv and is_3d and keep_dispc:
            ...
            self.conv1 = BasicConv(
                in_channels, out_channels, deconv, is_3d, bn=True, relu=True, kernel_size=kernel, stride=2, padding=1
            )
...
def convbn_3d(in_channels, out_channels, kernel_size, stride, pad):
    return nn.Sequential(
        nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=pad, bias=False),
...
def convbn_3d_group(in_channels, out_channels, groups, kernel_size, stride, pad):
    return nn.Sequential(
        nn.Conv3d(
            in_channels, out_channels, groups=groups, kernel_size=kernel_size, stride=stride, padding=pad, bias=False
        ),
        nn.BatchNorm3d(out_channels),
    )
...
def groupwise_correlation_volume(volume1, volume2, num_groups):
    B, C, D, H, W = volume1.shape
    assert C % num_groups == 0
    channels_per_group = C // num_groups
    cost = (volume1 * volume2).view([B, num_groups, channels_per_group, D, H, W]).mean(dim=2)
...
def build_gwc_volume_norm(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            volume[:, :, i, :, i:] = groupwise_correlation_norm(
                refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i], num_groups
            )
        else:
...
def patch_aggregation(gwc_volume, patch_weight):
    ...
    gwc_volume_pad_unfold = (
        gwc_volume_pad_unfold.contiguous()
        .view(
            gwc_volume.shape[0], gwc_volume.shape[1], gwc_volume.shape[2], gwc_volume.shape[3], gwc_volume.shape[4], -1
        )
        .permute(0, 1, 5, 2, 3, 4)
    )
    gwc_volume_pad_unfold = (
        patch_weight.view(
            gwc_volume.shape[0], gwc_volume.shape[1], 1, gwc_volume.shape[2], gwc_volume.shape[3], gwc_volume.shape[4]
        )
        * gwc_volume_pad_unfold
    )
...
def build_gwc_volume_v1(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            volume[:, :, i, :, (2 * i) :] = groupwise_correlation(
                refimg_fea[:, :, :, (2 * i) :], targetimg_fea[:, :, :, : -(2 * i)], num_groups
            )
        else:
            volume[:, :, i, :, :] = groupwise_correlation(refimg_fea, targetimg_fea, num_groups)
    volume = volume.contiguous()
...
class BasicBlock_gn(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, downsample, pad, dilation):
        ...
        self.conv1 = nn.Sequential(convgn(inplanes, planes, 3, stride, pad, dilation), nn.ReLU(inplace=True))
...
class BasicBlock_groups(nn.Module):
    ...
    def __init__(self, inplanes, planes, groups, stride, downsample, pad, dilation):
        ...
        self.conv1 = nn.Sequential(
            convbn_group(inplanes, planes, groups, 3, stride, pad, dilation), nn.ReLU(inplace=True)
        )
...
class attention_block(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv_3d = (
            self.qkv_3d(x)
            .reshape(
                B, d * h * w, self.block[0] * self.block[1] * self.block[2], 3, self.num_heads, C // self.num_heads
            )
            .permute(3, 0, 1, 4, 2, 5)
        )  # [3,B,d*h*w,num_heads,blocks,C//num_heads]
        ...
        if pad_r > 0 or pad_b > 0:
            ...
            mask[:, :, -pad_r:].fill_(1)
            mask = (
                mask.reshape(1, h, self.block[1], w, self.block[2])
                .transpose(2, 3)
                .reshape(1, h * w, self.block[1] * self.block[2])
            )
        ...
        x = (
            (attn @ v_3d)
            .view(B, d, h, w, self.num_heads, self.block[0], self.block[1], self.block[2], -1)
            .permute(0, 4, 8, 1, 5, 2, 6, 3, 7)
        )
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, left_input, right_input, disparity_samples):
        ...
        right_y_coordinate = (
            left_y_coordinate.expand(disparity_samples.size()[1], -1, -1, -1).permute([1, 0, 2, 3]) - disparity_samples
        )

        right_y_coordinate_1 = right_y_coordinate
        right_y_coordinate = torch.clamp(right_y_coordinate, min=0, max=(right_input.size()[3] - 1))

        warped_right_feature_map = torch.gather(
            right_feature_map,
            dim=4,
        ...
        warped_right_feature_map = (
            1 - ((right_y_coordinate_1 < 0) + (right_y_coordinate_1 > right_input.size()[3] - 1)).float()
...
def SpatialTransformer_grid(x, y, disp_range_samples):
    ...
    y_warped = F.grid_sample(
        ...
        bs, channels, ndisp, height, width
    )  # (B, C, D, H, W)
