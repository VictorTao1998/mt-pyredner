...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=(1, 1), residual=True, BatchNorm=None):
        ...
        self.conv2 = nn.Conv2d(
            planes, planes, kernel_size=3, stride=stride, padding=dilation[1], bias=False, dilation=dilation[1]
        )
        self.bn2 = BatchNorm(planes)
...
class DRN(nn.Module):
    def __init__(self, block, layers, arch="D", channels=(16, 32, 64, 128, 256, 512, 512, 512), BatchNorm=None):
        ...
        if arch == "C":
            self.conv1 = nn.Conv2d(1, channels[0], kernel_size=7, stride=1, padding=3, bias=False)
        ...
        self.layer6 = (
            None
            if layers[5] == 0
            else self._make_layer(block, channels[5], layers[5], dilation=4, new_level=False, BatchNorm=BatchNorm)
        )
        ...
        if arch == "C":
            self.layer7 = (
                ...
                if layers[6] == 0
                    ...
                    BasicBlock, channels[6], layers[6], dilation=2, new_level=False, residual=False, BatchNorm=BatchNorm
                )
            )
            self.layer8 = (
                None
                if layers[7] == 0
                    ...
                    BasicBlock, channels[7], layers[7], dilation=1, new_level=False, residual=False, BatchNorm=BatchNorm
                )
            )
        elif arch == "D":
            self.layer7 = (
                None
                if layers[6] == 0
                else self._make_conv_layers(channels[6], layers[6], dilation=2, BatchNorm=BatchNorm)
            )
            self.layer8 = (
                None
                if layers[7] == 0
                else self._make_conv_layers(channels[7], layers[7], dilation=1, BatchNorm=BatchNorm)
            )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, new_level=True, residual=True, BatchNorm=None):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, residual=residual, dilation=(dilation, dilation), BatchNorm=BatchNorm)
            )
