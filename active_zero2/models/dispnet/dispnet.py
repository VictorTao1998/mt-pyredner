...
class DispNet(torch.nn.Module):
    def __init__(self, max_disp: int):
        ...
        # encoder part start
        self.conv1 = torch.nn.Conv2d(in_channels=2, out_channels=64, kernel_size=7, stride=2, padding=3)
    ...
    def forward(self, data_batch):
        ...
        pr1_up = F.upsample(pr1, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr2_up = F.upsample(pr2, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr3_up = F.upsample(pr3, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr4_up = F.upsample(pr4, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr5_up = F.upsample(pr5, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr6_up = F.upsample(pr6, (x.shape[-2], x.shape[-1]), mode="bilinear", align_corners=False)
        pr0 = F.sigmoid(pr0) * self.max_disp
        pr1_up = F.sigmoid(pr1_up) * self.max_disp
        pr2_up = F.sigmoid(pr2_up) * self.max_disp
        pr3_up = F.sigmoid(pr3_up) * self.max_disp
        pr4_up = F.sigmoid(pr4_up) * self.max_disp
        pr5_up = F.sigmoid(pr5_up) * self.max_disp
        pr6_up = F.sigmoid(pr6_up) * self.max_disp
    ...
    def compute_disp_loss(self, data_batch, pred_dict):
        ...
        loss_disp = 0.0
        for k, disp_pred in pred_dict.items():
            loss_disp += F.smooth_l1_loss(disp_pred[mask], disp_gt[mask], reduction="mean")
