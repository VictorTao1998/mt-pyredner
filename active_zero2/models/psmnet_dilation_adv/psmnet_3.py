...
class Discriminator(nn.Module):
    def __init__(self, in_channels, base_channels, bias=False):
        ...
        self.net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, True),
            conv3d_half(base_channels * 4, base_channels * 8, bias),
            nn.LeakyReLU(0.2, True),
            conv3d_minus3(base_channels * 8, 1, bias),
        )
...
class PSMNetADV(nn.Module):
    def __init__(
        ...
        self.disp_list = torch.linspace(min_disp, max_disp, num_disp)
        self.T = (max_disp - min_disp) / (num_disp - 1)
        self.disp_list_4 = torch.linspace(min_disp, max_disp, self.num_disp_4) / 4
        ...
        for t in disp_encoding:
            disp_encoded.append(torch.sin(self.disp_list * t))
            disp_encoded.append(torch.cos(self.disp_list * t))
        if disp_encoded:
    ...
    def D_backward(self, data_batch, pred_dict):
        ...
        err_d_real.backward()
        pred_prob_volume = pred_dict["prob_cost3"].unsqueeze(1)
        if self.disp_encoded is not None:
