...
class DisparityRegression(nn.Module):
    def __init__(self, min_disp, max_disp, num_disp):
...
class FeatureExtraction(nn.Module):
    def __init__(self, in_channels: int, base_channels: int):
        ...
        self.firstconv = nn.Sequential(
            convbn(in_channels, base_channels, 3, 2, 1, 1),
            nn.ReLU(inplace=True),
            convbn(base_channels, base_channels, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            convbn(base_channels, base_channels, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
        )
        self.layer1 = self._make_layer(BasicBlock, base_channels, 3, 1, 1, 1)  # conv1_x
        self.layer2 = self._make_layer(BasicBlock, base_channels*2, 16, 2, 1, 1)  # conv2_x
        self.layer3 = self._make_layer(BasicBlock, base_channels*4, 3, 1, 1, 1)  # conv3_x
        self.layer4 = self._make_layer(BasicBlock, base_channels*4, 3, 1, 1, 2)  # conv4_x

        # SPP module
        self.branch1 = nn.Sequential(
            nn.AvgPool2d((64, 64), stride=(64, 64)),
            convbn(base_channels*4, base_channels, 1, 1, 0, 1),
            nn.ReLU(inplace=True),
        )
        ...
        self.lastconv = nn.Sequential(
            ...
            convbn(base_channels*10, base_channels*4, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_channels * 4, base_channels, kernel_size=1, padding=0, stride=1, bias=False),
        )
