...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.Adam(
        [diff_scene.epsilon, diff_scene.mu, diff_scene.sigma, diff_scene.beta] + list(
            diff_scene.cnn_1.parameters()) + list(diff_scene.cnn_2.parameters()),
        lr=0.001)
    ...
    for epoch_idx in range(1, NUM_EPOCHS + 1):
        ...
        for i in tqdm(range(num_sample)):
            ...
            depth_loss, grad_loss = compute_loss(render_dict)
            loss_depth += depth_loss.item()
            loss_grad += grad_loss.item()

            total_loss = depth_loss + GRAD_WEIGHT * grad_loss
            total_loss.backward()
            optimizer.step()
